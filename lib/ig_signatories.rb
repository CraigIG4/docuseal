...
module IgSignatories
  ...
  ENTITIES = {
    iti: {
      name: 'Ignition Telecoms Investments (Pty) Ltd', short_name: 'ITI',
      ...
      final_signatory_operational: :donovan_bergsma, final_signatory_other: :sean_bergsma,
    },
    comit: {
      name: 'Comit Technologies (Pty) Ltd', short_name: 'COMIT',
    ...
    mvnx: {
      name: 'MVN-X (Pty) Ltd', short_name: 'MVN-X',
    ...
    uconnect: {
      name: 'UConnect Mobile (Pty) Ltd', short_name: 'UConnect',
    ...
    ccs: {
      name: 'CCS Outsourcing (Pty) Ltd', short_name: 'CCS Outsourcing',
    ...
    chase_tracking: {
      name: 'Chase Tracking (Pty) Ltd', short_name: 'Chase Tracking',
    ...
    ignite_training: {
      name: 'Ignite Training Academy (Pty) Ltd', short_name: 'Ignite Training',
    ...
    me_and_you: {
      name: 'Me and You Mobile (Pty) Ltd', short_name: 'Me & You Mobile',
    ...
    mobius: {
      name: 'Mobius Mobile Telecommunications (Pty) Ltd', short_name: 'Mobius',
    ...
    ucingo: {
      name: 'Ucingo Administration 321 (Pty) Ltd', short_name: 'Ucingo',
    ...
    all_sevens: {
      name: 'All Sevens Trade and Invest (Pty) Ltd', short_name: 'All Sevens',
    ...
    benjistar: {
      name: 'Benjistar (Pty) Ltd', short_name: 'Benjistar',
    ...
    so_music: {
      name: 'So Music Industries (Pty) Ltd', short_name: 'So Music',
  ...
  def self.chain_for(caf_type, entity_key)
    ...
    chain = (bu_signers + [entity[:bu_finance], final]).compact.uniq.map do |k|
      p = PEOPLE[k]; next unless p
      { key: k, name: p[:name], title: p[:title], email: p[:email] }
    end.compact
    { stage1: chain, stage2: [] }
  end

  def self.entity_details(entity_key) = ENTITIES[entity_key.to_sym]
  def self.person(person_key) = PEOPLE[person_key.to_sym]
  def self.entities_for_js
    ENTITIES.map { |key, e|
      { key: key.to_s, name: e[:name], short_name: e[:short_name],
        registration: e[:registration], address: e[:address] }
    }
  end
end
